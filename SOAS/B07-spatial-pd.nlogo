patches-own [
  strategy reward cooperated betrayed last_time tmp_strategy ;; the current strategy: "random", "always cooperate", "always defect", "grim trigger", "tranquilizer", "tit for tat"
]

to setup
  clear-all

  let possible-strategies [ ]

  if strategy_random [
   set possible-strategies fput "random" possible-strategies
  ]

  if strategy_cooperate [
   set possible-strategies fput "always cooperate" possible-strategies
  ]

  if strategy_defect [
   set possible-strategies fput "always defect" possible-strategies
  ]

  if strategy_tranquilize [
   set possible-strategies fput "tranquilizer" possible-strategies
  ]

  if strategy_grim_trigger [
   set possible-strategies fput "grim trigger" possible-strategies
  ]

  if strategy_tit_for_tat [
   set possible-strategies fput "tit for tat" possible-strategies
  ]
...
to go
  ...
  if ticks mod 8 = 0 and ticks != 0 [
    ask patches [select-strategy]   ;; select the strategy of the neighbor who had the highest score in the last 8 rounds
    ask patches [apply-new-strategy] ;; apply selected strategy
  ]
...
to interact  ;; patch procedure
  ...
  ;; select choice for current patch
  let choice 1
  let choice_neighbour 1
  let reward_self 0
  ...
  if strategy = "tranquilizer" [
    ifelse cooperated != 0
    [
      ifelse random-float 2 > betrayed / cooperated
       [
       set choice 0
      ]
      [
  ...
  ;; select choice for neighbour patch
   if strategy = "random" [
    set choice_neighbour random 2
   ]

   if strategy = "always cooperate" [
    set choice_neighbour 1
   ]

   if strategy = "always defect" [
    set choice_neighbour 0
   ]
   ...
   if choice = 1 and choice_neighbour = 1 [
     set reward reward + payoff-r
     set reward_self payoff-r
   ]
   if choice = 1 and choice_neighbour = 0 [
     set reward reward + payoff-t
     set reward_self payoff-s
   ]
   if choice = 0 and choice_neighbour = 1 [
     set reward reward + payoff-s
     set reward_self payoff-t
   ]
   if choice = 0 and choice_neighbour = 0 [
     set reward reward + payoff-p
     set reward_self payoff-p
   ]

   ;; update reward for neighbour patch
   ifelse choice = 1
   [
     set cooperated cooperated + 1
   ]
   [
     set betrayed betrayed + 1
   ]
   set last_time choice
  ]

;; update reward of current patch
set reward reward + reward_self
ifelse choice_neighbour = 1
[
  set cooperated cooperated + 1
]
[
...
set last_time choice_neighbour
end

to select-strategy  ;; patch procedure
  let best_strategy "random"
  let best_reward 0

  ask neighbors[
    if reward > best_reward [
      set best_reward reward
      set best_strategy strategy
    ]
  ]
  if keep_own_strategy [
    if reward > reward
    [
      set best_strategy strategy
    ]
  ...
  set tmp_strategy best_strategy
end

to apply-new-strategy
 set strategy tmp_strategy
 set reward 0
 set cooperated 0
 set betrayed 0
 set last_time 1
